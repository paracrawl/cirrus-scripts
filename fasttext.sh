...
# export PATH=/home/cs-sifa1/fastText/:$PATH
# export fasttext_lib=/home/cs-sifa1/fastText/lang_detection/lib.176.bin
DOCLANG=${1:-"bg"}
FILE_IN=${2:-"test_input.txt"}
echo $DOCLANG , $FILE_IN
paste "$FILE_IN" <(fasttext predict /home/cs-sifa1/fastText/lang_detection/lid.176.bin "$FILE_IN")\
            | sed -r "s/__label__//" \
                        | gawk -F$"\t" -v DLANG="$DOCLANG" '{if ($2==DLANG) {print $1}}' 

                        # | gawk -F$"\t" -v DLANG="$DOCLANG" '{if ($2==DLANG) {print $1}}' > outfile.txt
