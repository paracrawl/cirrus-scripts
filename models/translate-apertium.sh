...
translate-apertium() {
	...
	| apertium -f html -u $MODEL \
	| sed -r 's/<\/?p>//g' \
	| $PERL -CS -MHTML::Entities -pe 'decode_entities($_);'
}
...
parallel --halt 2 -j $THREADS --pipe -k -l $RECSIZE $SCRIPTS/bisector.py bash -c translate-apertium "$@"
